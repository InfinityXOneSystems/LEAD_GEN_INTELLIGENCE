...
export const leadsApi = {
  ...
  create: (data: Partial<Contractor>) => apiClient.post<Contractor>('/leads', data),
  update: (id: string, data: Partial<Contractor>) => apiClient.put<Contractor>(`/leads/${id}`, data),
...
export const scrapersApi = {
  createJob: (data: { query?: string; city?: string; state?: string; industry?: string }) =>
  ...
  cancelJob: (id: string) => apiClient.post<ScrapeJob>(`/scrapers/jobs/${id}/cancel`),
...
export const agentsApi = {
  ...
  logs: (name: string) => apiClient.get<{ name: string; logs: string[] }>(`/agents/${name}/logs`),
};

export const commandsApi = {
  execute: (command: string) =>
    apiClient.post<{ action: string; parameters: Record<string, unknown>; job_id?: string; message: string }>(
      '/commands/execute',
      { command }
    ),
};
