  ...
  return (
    <div className="p-8 space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">Monitor and control autonomous agents</p>
      </div>
            ...
            <div key={agent.name} className="card">
              <div className="flex items-start justify-between">
                ...
                <span
                  className={`badge ${
                    ...
                    agent.status === 'error' ? 'badge-red' : 'badge-gray'
                  }`}
                >
                  {agent.status}
                </span>
              ...
              <div className="flex gap-2 mt-4">
                <button
                  ...
                  disabled={agent.status === 'running' || startMutation.isPending}
                ...
                <button
                  ...
                  disabled={agent.status !== 'running' || stopMutation.isPending}
        ...
        <div className="card">
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-sm font-semibold text-gray-700">
              {selectedAgent ? `Logs: ${selectedAgent.replace(/_/g, ' ')}` : 'Select an agent to view logs'}
            </h2>
            {selectedAgent && (
              <button
                onClick={() => qc.invalidateQueries({ queryKey: ['agent-logs', selectedAgent] })}
          ...
          <div className="bg-gray-900 rounded-lg p-4 h-80 overflow-y-auto font-mono text-xs text-green-400 space-y-1">
            {logsData?.logs.length ? (
              ...
              <span className="text-gray-500">
                {selectedAgent ? 'No logs yet' : 'Click "Logs" on an agent to view its activity'}
              </span>
            )}
          </div>
        </div>
