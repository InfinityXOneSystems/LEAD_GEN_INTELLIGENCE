...
const SANDBOX_TIMEOUT_MS = parseInt(process.env.SANDBOX_TIMEOUT_MS || '600000', 10); // 10 min

/** Ad/tracker domains to block inside sandboxes */
const BLOCK_DOMAINS = [
  'doubleclick.net', 'googlesyndication.com', 'google-analytics.com',
  'facebook.com/tr', 'pixel.facebook.com', 'analytics.twitter.com',
  'ads.twitter.com', 'hotjar.com', 'segment.io', 'mixpanel.com',
];
...
class SandboxManager {
  ...
  async createSandbox(options = {}) {
    ...
    if (options.logRequests !== false) {
      page.on('request', req => {
        log.push({ ts: Date.now(), type: 'request', method: req.method(), url: req.url() });
      });
      page.on('response', resp => {
        log.push({ ts: Date.now(), type: 'response', status: resp.status(), url: resp.url() });
      });
    }
    ...
    const timer = setTimeout(async () => {
      console.warn(`[SandboxManager] Timeout — destroying sandbox: ${sandboxId}`);
      await this.destroySandbox(sandboxId);
    }, SANDBOX_TIMEOUT_MS);

    this._sandboxes.set(sandboxId, { context, page, log, timer });
    console.log(`[SandboxManager] Sandbox created: ${sandboxId} (total: ${this._sandboxes.size})`);
  ...
  async destroySandbox(sandboxId) {
    ...
    try { await s.context.close(); } catch (_) { /* ignore */ }
    this._sandboxes.delete(sandboxId);
    console.log(`[SandboxManager] Sandbox destroyed: ${sandboxId} (remaining: ${this._sandboxes.size})`);
    return true;
  }
