      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <div className="flex gap-2">
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={() => { setSearch(''); setStatusFilter('all'); refresh() }}
            title="Refresh from API"
          >
            {loading ? <SpinnerGap size={16} className="animate-spin" /> : <ArrowsClockwise size={16} />}
          </Button>
        </div>
                  ...
                  >
                    <td className="p-4">
                      ...
                      {contractor.name && contractor.name !== contractor.company && (
                        <div className="text-xs text-muted-foreground mt-0.5">{contractor.name}</div>
                      )}
                    </td>
                    <td className="p-4">
                      {contractor.email && (
                        <div className="flex items-center gap-1 text-muted-foreground">
                          <Envelope size={12} />
                          <a href={`mailto:${contractor.email}`} className="text-xs hover:text-primary">
                            {contractor.email}
                          </a>
                        </div>
                      )}
                    ...
                    <td className="p-4">
                      <span className={`text-xs px-2 py-1 rounded-full border ${STATUS_COLORS[contractor.status] || STATUS_COLORS.new}`}>
                        {contractor.status}
                      </span>
                    </td>
                    <td className="p-4">
                    ...
                    <td className="p-4">
                      <span className="text-xs text-muted-foreground capitalize">
                        {(contractor.source || 'shadow_scraper').replace(/_/g, ' ')}
                      </span>
                    </td>
                  </motion.tr>
                ))}
              ...
              <Card className="hover:border-primary/30 transition-colors cursor-pointer">
                ...
                <CardContent className="space-y-2">
                  ...
                  <div className="flex items-center justify-between mt-2">
                    <span className="text-xs text-muted-foreground">{contractor.category}</span>
                    <span className={`text-xs px-2 py-0.5 rounded-full border ${STATUS_COLORS[contractor.status] || STATUS_COLORS.new}`}>
                      {contractor.status}
                    </span>
                  </div>
                </CardContent>
