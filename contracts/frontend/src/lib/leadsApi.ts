...
function normalise(r: RawLead, idx: number): Lead {
  ...
  const tier = (
    r.tier || (score >= 75 ? "hot" : score >= 50 ? "warm" : "cold")
  ).toLowerCase();
  const created =
    r.date_scraped || r.scrapedAt || r.createdAt || new Date().toISOString();
...
export const leadsApi = {
  async getAll(): Promise<Lead[]> {
    try {
      ...
      console.warn(
        "[leadsApi] Railway API unavailable, loading static fallback:",
        err,
      );
    }
