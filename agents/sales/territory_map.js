...
class TerritoryMapEngine {
  ...
  getLeadsByState() {
    ...
    for (const lead of leads) {
      ...
      if (!result[state]) result[state] = { state: lead.state || 'Unknown', count: 0, leads: [] };
      result[state].count += 1;
      if (result[state].leads.length < 5) result[state].leads.push(lead.company_name || lead.name);
    }
    return result;
  }
  ...
  getLeadsByCity() {
    ...
    for (const lead of leads) {
      ...
      cityMap[key].count += 1;
      if (cityMap[key].leads.length < 5) cityMap[key].leads.push(lead.company_name || lead.name);
    }
  ...
  getTerritoryStats(state) {
    ...
    if (!stateLeads.length) return { state, count: 0, cities: {}, industries: {} };
    ...
    return {
      ...
      averageScore: scoredCount > 0 ? (totalScore / scoredCount).toFixed(1) : null,
      topCity: Object.entries(cities).sort((a, b) => b[1] - a[1])[0]?.[0] || null,
      cities,
      industries,
    };
  }
