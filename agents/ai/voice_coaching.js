...
const KEYWORDS = {
  objection_handling: [
    'not interested', 'too expensive', 'already have', 'no budget', 'call back later',
    'think about it', 'send me info', 'not right now', 'busy', 'don\'t need',
  ],
  rapport: [
    'how are you', 'great question', 'understand', 'appreciate', 'i hear you',
    'makes sense', 'absolutely', 'totally', 'definitely', 'of course',
  ],
  next_step: [
    'follow up', 'next week', 'schedule', 'meeting', 'demo', 'call back',
    'appointment', 'calendar', 'tuesday', 'wednesday', 'thursday', 'time works',
  ],
  pricing: [
    'price', 'cost', 'how much', 'fee', 'charge', 'investment', 'budget',
    'afford', 'expensive', 'cheap', 'discount', 'payment',
  ],
};
...
function detectSignals(transcript) {
  ...
  return {
    objection_handling: KEYWORDS.objection_handling.filter((kw) => lower.includes(kw)),
...
function ruleBasedAnalysis(transcript, leadData = {}) {
  ...
  if (signals.objection_handling.length > 0 && signals.rapport.length > 0) {
    ...
    score += 10;
  } else if (signals.objection_handling.length > 0 && signals.rapport.length === 0) {
    improvements.push('Objections were raised but little rapport language detected. Try empathy phrases.');
    score -= 5;
  }

  // Rapport building
  if (signals.rapport.length >= 3) {
    strengths.push('Strong rapport-building language used throughout the call.');
    score += 10;
  } else if (signals.rapport.length === 0) {
    improvements.push('No rapport-building language detected. Use phrases like "I understand" or "That makes sense".');
    score -= 10;
  }
  ...
  if (signals.next_step.length > 0) {
    ...
    improvements.push('No clear next step identified in the transcript. Always close with a specific commitment.');
    score -= 15;
    feedback.push('Critical: Always end the call with a confirmed next action (demo, callback, email).');
  }
  ...
  if (wordCount < 100) {
    improvements.push('Call appears very short. Consider engaging the prospect more before pitching.');
    score -= 10;
...
${transcript}`;
    ...
    try {
      ...
      console.error('[VoiceCallCoachingAI] OpenAI error, falling back to rule-based:', err.message);
      return ruleBasedAnalysis(transcript, leadData);
    }
  }
  ...
  async generateCoachingTips(repStats = {}) {
    ...
    const closeRate = total_calls > 0 ? ((closed / total_calls) * 100).toFixed(1) : 0;
...
Provide 5 specific, actionable coaching tips to improve this rep's performance in flooring/construction sales. Be direct and practical.`;
        ...
        return { tips: completion.choices[0].message.content.trim(), stats: repStats };
      } catch (err) {
        console.error('[VoiceCallCoachingAI] Coaching tips OpenAI error:', err.message);
      }
    }

    // Rule-based coaching tips
    const tips = [];
    if (avg_score < 60) tips.push('Focus on rapport: open every call with a genuine question about their business.');
    if (parseFloat(closeRate) < 20) tips.push('Improve your close rate by always confirming a specific next step before ending the call.');
    if (objections_handled < total_calls * 0.5) tips.push('Practice objection handling with the "Feel, Felt, Found" framework.');
    if (follow_ups_set < total_calls * 0.7) tips.push('Book follow-ups before ending the call — never rely on "I\'ll call you."');
    tips.push('Record and review your top-performing calls weekly to reinforce winning patterns.');
