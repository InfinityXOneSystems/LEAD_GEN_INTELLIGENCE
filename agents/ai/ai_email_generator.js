...
class AIEmailGenerator {
  ...
  async generateEmail(lead = {}, emailType = 'cold_outreach', customPrompt = null) {
    if (this._openai) {
...
Always respond with valid JSON only: {"subject": "...", "body": "..."}`;
    ...
    const userPrompt = customPrompt || `Write a ${emailType.replace(/_/g, ' ')} email for:
...
Keep the email under 150 words. Sound human and genuine. No fluff.`;
    ...
    try {
      ...
      const subject = parsed.subject || `Outreach for ${lead.name || 'your business'}`;
      const body = parsed.body || raw;
      return { subject, body, html: buildHtml(subject, body), plain_text: body };
    } catch (err) {
      console.error('[AIEmailGenerator] OpenAI error, falling back to template:', err.message);
      return this._generateFromTemplate(lead, emailType);
    }
  }
