...
class PromptRegistry {
  ...
  getPrompt(name, variables = {}) {
    ...
    if (!entry) {
      throw new Error(`Prompt "${name}" not found. Use listPrompts() to see available prompts.`);
    }
    return {
      name,
  ...
  addPrompt(name, template, description = '') {
    if (BUILTIN_PROMPTS[name]) {
      throw new Error(`Cannot overwrite builtin prompt "${name}" with addPrompt. Use updatePrompt().`);
    }
    this._custom[name] = {
      template,
      description,
