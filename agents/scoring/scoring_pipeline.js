...
function loadLeads() {
  if (!fs.existsSync(LEADS_PATH)) {
    console.warn(`[scoring_pipeline] No leads file found at ${LEADS_PATH}. Using empty array.`);
    return [];
  }
  try {
    ...
    if (!Array.isArray(leads)) {
      console.warn('[scoring_pipeline] leads.json does not contain an array. Using empty array.');
      return [];
    }
    return leads;
  } catch (err) {
    console.error('[scoring_pipeline] Failed to parse leads.json:', err.message);
    return [];
  }
}
...
function runPipeline() {
  ...
  console.log(`[scoring_pipeline] Tiers -> HOT: ${tiers.HOT.length}  WARM: ${tiers.WARM.length}  COLD: ${tiers.COLD.length}`);
  console.log(`[scoring_pipeline] Industries -> ${Object.entries(industries).map(([k, v]) => `${k}: ${v.length}`).join(', ')}`);
