...
const IDLE_TIMEOUT_MS = parseInt(process.env.HEADLESS_IDLE_TIMEOUT_MS || '300000', 10); // 5 min
...
class HeadlessBrowserPool {
  ...
  async createSession(options = {}) {
    ...
    if (this._sessions.size >= MAX_SESSIONS) {
      ...
      if (!freed) {
        throw new Error(`Max sessions reached (${MAX_SESSIONS}). All slots busy — try again later.`);
      }
    }
    ...
    console.log(`[HeadlessPool] Session created: ${id} (active: ${this._sessions.size})`);
    return id;
  }
  ...
  async destroySession(id) {
    ...
    console.log(`[HeadlessPool] Session destroyed: ${id} (active: ${this._sessions.size})`);
    return true;
  }
