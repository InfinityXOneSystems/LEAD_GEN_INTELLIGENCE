...
router.get('/health', (_req, res) => {
  res.json({ status: 'ok', service: 'headless-agent', activeSessions: pool.listSessions().length });
});
...
router.post('/click', async (req, res) => {
  const { sessionId, selector, button = 'left', clickCount = 1, modifiers = [] } = req.body || {};
  if (!sessionId || !selector) return err(res, 400, 'sessionId and selector required');
  try {
    const { page } = pool.getSession(sessionId);
...
router.post('/evaluate', async (req, res) => {
  ...
  if (!sessionId || !expression) return err(res, 400, 'sessionId and expression required');
