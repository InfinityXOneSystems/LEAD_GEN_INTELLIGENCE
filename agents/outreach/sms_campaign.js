...
const CAMPAIGNS_FILE = path.join(__dirname, '../../data/outreach/sms_campaigns.json');
...
class SMSCampaignEngine {
  ...
  async sendSMS(to, message) {
    ...
    try {
      ...
      console.error(`[SMSCampaignEngine] Failed to send SMS to ${to}:`, logEntry.error);
    }
  ...
  async runCampaign(campaignId) {
    ...
    for (const lead of campaign.leads) {
      ...
      if (result.status === 'stub' || result.status === 'sent' || result.status === 'queued') {
