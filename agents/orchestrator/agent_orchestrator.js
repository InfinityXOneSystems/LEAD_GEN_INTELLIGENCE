...
class AgentOrchestrator {
  ...
  _loadValidator() {
    return safeLazyLoad(path.join(ROOT, 'agents/validation/validation_engine')) ||
  ...
  async runFullPipeline(options = {}) {
    log('=== Starting full pipeline: scrape → validate → deduplicate → enrich → score → export ===');
    ...
    const validateStep = await this._runStep('validate', async () => {
      ...
      if (!Validator) { log('Validator not available – skipping', 'WARN'); return { skipped: true }; }
      const v = typeof Validator === 'function' && Validator.prototype ? new Validator() : Validator;
    ...
    const dedupStep = await this._runStep('deduplicate', async () => {
      ...
      if (!Dedup) { log('Deduplicator not available – skipping', 'WARN'); return { skipped: true }; }
      const d = typeof Dedup === 'function' && Dedup.prototype ? new Dedup() : Dedup;
    ...
    const enrichStep = await this._runStep('enrich', async () => {
      const leadsFile = path.join(ROOT, 'leads/leads.json');
      const leads = this._loadLeads(leadsFile);
      return await this.runEnrichmentPipeline(leads.slice(0, options.enrichLimit || 50));
    });
    ...
    const scoreStep = await this._runStep('score', async () => {
      ...
      if (!ScoringPipeline) { log('ScoringPipeline not available – skipping', 'WARN'); return { skipped: true }; }
      const sp = typeof ScoringPipeline === 'function' && ScoringPipeline.prototype ? new ScoringPipeline() : ScoringPipeline;
    ...
    const exportStep = await this._runStep('export', async () => {
      ...
      if (!Exporter) { log('Exporter not available – skipping', 'WARN'); return { skipped: true }; }
      const e = typeof Exporter === 'function' && Exporter.prototype ? new Exporter() : Exporter;
    ...
    log(`=== Pipeline complete in ${totalMs}ms — ${summary.stepsPassed}/${summary.stepsTotal} steps passed ===`);

    // Persist run summary
    const summaryFile = path.join(DATA_DIR, 'logs', 'pipeline_runs.json');
    try {
      let runs = [];
      if (fs.existsSync(summaryFile)) runs = JSON.parse(fs.readFileSync(summaryFile, 'utf8'));
      runs.push(summary);
  ...
  async runEnrichmentPipeline(leads) {
    ...
    if (!EnrichmentEngine) {
      ...
      return { enriched: 0, skipped: leads.length, reason: 'EnrichmentEngine not available' };
    }

    log(`Running enrichment on ${leads.length} leads`);
    const engine = typeof EnrichmentEngine === 'function' && EnrichmentEngine.prototype
  ...
  async runOutreachPipeline(leads, campaign = {}) {
    ...
    const qualified = leads.filter((l) => l.email && (l.score || 0) >= (campaign.minScore || 30));
    log(`Outreach pipeline: ${qualified.length}/${leads.length} qualified leads`);
    ...
    if (fs.existsSync(queueFile)) {
      try { queue = JSON.parse(fs.readFileSync(queueFile, 'utf8')); } catch (_) {}
    }
    ...
    log(`Queued ${added.length} leads for outreach (campaign: ${campaign.name || 'default'})`);
    return { queued: added.length, skipped: leads.length - qualified.length };
  }

  getSystemStatus() {
    const agents = [
      { name: 'scoring_pipeline', path: path.join(ROOT, 'agents/scoring/scoring_pipeline') },
      { name: 'enrichment_engine', path: path.join(ROOT, 'agents/enrichment/enrichment_engine') },
      { name: 'deduplication_engine', path: path.join(ROOT, 'agents/dedupe/deduplication_engine') },
      { name: 'gpt_actions_server', path: path.join(ROOT, 'agents/gpt_actions/server') },
      { name: 'infinity_orchestrator', path: path.join(ROOT, 'agents/orchestrator/infinity_orchestrator') },
      { name: 'system_monitor', path: path.join(ROOT, 'agents/monitor/system_monitor') },
      { name: 'full_system_monitor', path: path.join(ROOT, 'agents/monitor/full_system_monitor') },
