...
const ALLOWED_STAGES = new Set(['scrape', 'validate', 'score', 'outreach', 'export', 'pipeline']);
...
function runPipelineStage(stage) {
  return new Promise((resolve, reject) => {
    ...
    execFile(cmd, args, { cwd: root, timeout: 120_000 }, (err, stdout, stderr) => {
...
function parseCommand(body = '') {
  const match = body.match(/^\/(scrape|validate|score|outreach|export|pipeline)\b/im);
  return match ? match[1].toLowerCase() : null;
}

/* ───────────────────────── webhook route ───────────────────────── */
app.post('/webhooks/infinity-orchestrator', webhookRateLimiter, async (req, res) => {
  ...
  try {
    ...
    if (event === 'issue_comment' && payload.action === 'created') {
      ...
      if (cmd) {
        ...
        return res.json({ ok: true, command: cmd, output: out.slice(0, 500) });
      }
    }
    ...
    if (event === 'workflow_run' && payload.workflow_run?.conclusion === 'failure') {
      ...
      console.warn(`[Orchestrator] Workflow failed: ${name} — consider auto-filing an issue`);
...
app.get('/health', (_req, res) => {
  res.json({ status: 'ok', service: 'infinity-orchestrator', ts: new Date().toISOString() });
});
