...
const FLOORING_KEYWORDS = {
  ...
  refinishing: ['refinish', 'refinishing', 'sand and finish', 'floor restoration'],
};

const CONSTRUCTION_KEYWORDS = {
  general: ['general contractor', 'general contracting', 'construction', 'remodel', 'remodeling', 'renovation'],
  roofing: ['roofing', 'roof repair', 'roof installation', 'shingles'],
  painting: ['painting', 'interior painting', 'exterior painting', 'paint contractor'],
    ...
    for (const [spec, keywords] of Object.entries(FLOORING_KEYWORDS)) {
      if (keywords.some((kw) => text.includes(kw))) {
        ...
        if (specializations.length === 1) {
          ...
          secondary_services.push(`${spec.charAt(0).toUpperCase() + spec.slice(1)} Flooring`);
        }
      }
    }
