...
class CaptchaSolver {
  constructor() {
    this.apiKey2Captcha = process.env.TWOCAPTCHA_API_KEY || process.env['2CAPTCHA_API_KEY'] || '';
  }

  async solve2Captcha(sitekey, url) {
    if (!this.apiKey2Captcha) throw new Error('[CaptchaSolver] 2CAPTCHA_API_KEY not set');

    console.info(`[CaptchaSolver] Submitting reCAPTCHA for ${url}`);
    const submitRes = await axios.get('https://2captcha.com/in.php', {
      params: { key: this.apiKey2Captcha, method: 'userrecaptcha', googlekey: sitekey, pageurl: url, json: 1 },
    });
    if (submitRes.data.status !== 1) {
      throw new Error(`[CaptchaSolver] Submission failed: ${submitRes.data.request}`);
    }
    ...
    while (Date.now() < deadline) {
      ...
      const pollRes = await axios.get('https://2captcha.com/res.php', {
        params: { key: this.apiKey2Captcha, action: 'get', id: taskId, json: 1 },
      });
  ...
  async solveHCaptcha(sitekey, url) {
    ...
    const submitRes = await axios.get('https://2captcha.com/in.php', {
      params: { key: this.apiKey2Captcha, method: 'hcaptcha', sitekey, pageurl: url, json: 1 },
    });
    if (submitRes.data.status !== 1) {
      throw new Error(`[CaptchaSolver] hCaptcha submission failed: ${submitRes.data.request}`);
    }
    const taskId = submitRes.data.request;
  ...
  async detectCaptcha(page) {
    try {
      ...
      if (/grecaptcha|recaptcha\.net|google\.com\/recaptcha/i.test(html)) return 'recaptcha';
  ...
  async autoSolve(page) {
    ...
    if (!sitekey) {
      console.warn('[CaptchaSolver] Could not extract sitekey — skipping auto-solve');
      return null;
    }
