...
function statusEmoji(s) {
  return { queued: "⏳", pending: "⏳", running: "⚡", completed: "✅", failed: "❌", retrying: "🔄", idle: "��", starting: "🚀", error: "❌" }[s] || "❓";
}
  ...
  return (
    <div style={S.taskPanel}>
      <div style={S.taskHeader}>
        <span style={{ ...S.statusDot, background: statusColor(task.status) }} />
        <span style={S.taskLabel}>Task {task.task_id ? task.task_id.slice(0, 8) : "…"}</span>
        <span style={{ ...S.statusText, color: statusColor(task.status) }}>{task.status?.toUpperCase()}</span>
      </div>
      {task.agent && <div style={S.taskMeta}>agent: {task.agent}</div>}
      {task.logs && task.logs.length > 0 && (
        <div style={S.taskLogs}>
          {task.logs.slice(-4).map((l, i) => <div key={i} style={S.logLine}>{l}</div>)}
        </div>
      )}
      {task.result && (
  ...
  return (
    <div style={S.toolStep}>
      <div style={S.toolStepHeader} onClick={() => setExpanded((v) => !v)}>
        <span style={{ color: statusColor(step.status), fontSize: "0.8rem" }}>{statusEmoji(step.status)}</span>
        <span style={S.toolStepLabel}>{step.label}</span>
        <span style={{ color: "#555", fontSize: "0.7rem", marginLeft: "auto" }}>{ago(step.timestamp)}</span>
        <span style={{ color: "#444", marginLeft: "0.4rem" }}>{expanded ? "▾" : "▸"}</span>
      </div>
  ...
  return (
    <div style={{ ...S.workerSlot, borderColor: statusColor(slot.status) }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "0.25rem" }}>
        <span style={{ color: statusColor(slot.status), fontSize: "0.75rem", fontWeight: 700 }}>
          {statusEmoji(slot.status)} {slot.status.toUpperCase()}
        </span>
        <span style={{ color: "#555", fontSize: "0.65rem" }}>{slot.task_id?.slice(0, 8)}</span>
        {onStop && slot.status === "running" && (
          <button style={S.stopBtn} onClick={() => onStop(slot.task_id)}>■</button>
        )}
      </div>
      <div style={{ color: "#aaa", fontSize: "0.72rem", overflow: "hidden", whiteSpace: "nowrap", textOverflow: "ellipsis" }}>
        {slot.command?.slice(0, 48)}
      </div>
      {slot.agent && (
        <div style={{ color: "#555", fontSize: "0.65rem", marginTop: "0.1rem" }}>agent: {slot.agent}</div>
      )}
    </div>
  );
}
...
function AgentSidebarItem({ agent, onStart }) {
  return (
    <div style={S.agentItem}>
      <div style={{ ...S.statusDot, background: statusColor(agent.status), margin: "0 0.4rem 0 0" }} />
      <span style={{ color: "#ccc", fontSize: "0.78rem", flex: 1 }}>{agent.name}</span>
      <span style={{ color: "#555", fontSize: "0.65rem", marginRight: "0.4rem" }}>{agent.status}</span>
      {agent.status !== "running" && (
        <button style={S.agentStartBtn} onClick={() => onStart(agent.name)}>▶</button>
      )}
    </div>
  );
}
...
function MessageBubble({ msg, onSuggestion }) {
  ...
  return (
    <div style={{ ...S.messageRow, justifyContent: isUser ? "flex-end" : "flex-start" }}>
      ...
      <div style={{
        ...
        {msg.toolSteps && msg.toolSteps.length > 0 && (
          <div style={{ marginTop: "0.5rem" }}>
            {msg.toolSteps.map((step, i) => <ToolCallStep key={i} step={step} idx={i} />)}
          </div>
        )}
        {msg.taskData && <TaskStatusPanel task={msg.taskData} />}
        {msg.fileEdit && (
          <div style={S.fileEditBadge}>
            📝 File edited: <code style={{ color: "#FFD700" }}>{msg.fileEdit.path}</code>
            <span style={{ color: "#4ade80", marginLeft: "0.5rem" }}>{msg.fileEdit.lines_written} lines</span>
          </div>
        )}
      </div>
      {isUser && <div style={{ ...S.avatar, background: "#FFD700", color: "#000" }}>U</div>}
    </div>
  );
}
...
export default function RuntimeCommandChat({ suggestions = [] }) {
  ...
  useEffect(() => {
    ...
    const fetchShadow = () => {
      fetch(`${api}/api/v1/runtime/shadow/status`)
        ...
        .then((d) => { if (d?.tasks) setShadowTasks(d.tasks); })
        .catch(() => {});
    };
    fetchShadow();
  ...
  const pollWorkerSlot = useCallback((taskId) => {
    ...
    const tick = async () => {
      if (attempts >= MAX_POLL_ATTEMPTS) {
        setWorkers((prev) => prev.map((w) => w.task_id === taskId ? { ...w, status: "failed" } : w));
        return;
      }
      attempts++;
      try {
        const r = await fetch(`${api}/api/v1/runtime/task/${taskId}`);
        if (!r.ok) { setTimeout(tick, POLL_INTERVAL_MS); return; }
        const d = await r.json();
        setWorkers((prev) => prev.map((w) => w.task_id === taskId ? { ...w, status: d.status, agent: d.agent } : w));
        if (!["completed", "failed"].includes(d.status)) setTimeout(tick, POLL_INTERVAL_MS);
    ...
    const tick = async () => {
      ...
      try {
        ...
        const toolSteps = (data.logs || []).map((log, i) => ({
          label: log,
          status: data.status === "failed" ? "failed" : i < (data.logs || []).length - 1 ? "completed" : data.status,
          timestamp: data.created_at,
          detail: null,
        }));
        setMessages((prev) => {
          const next = [...prev];
          if (next[msgIdx]) next[msgIdx] = { ...next[msgIdx], taskData: data, toolSteps };
          return next;
        });
  ...
  const runParallel = useCallback(async (cmds) => {
    const api = getApiBase();
    setShowWorkers(true);
    try {
      const resp = await fetch(`${api}/api/v1/runtime/parallel`, {
        ...
        body: JSON.stringify({ commands: cmds.map((c) => ({ command: c, priority: 6 })) }),
      });
      if (!resp.ok) return;
      const data = await resp.json();
      ...
      setWorkers((prev) => {
        const combined = [...prev.filter((w) => ["completed", "failed"].includes(w.status)), ...newSlots];
  ...
  const readFile = useCallback(async (path) => {
    const api = getApiBase();
    try {
      const r = await fetch(`${api}/api/v1/runtime/file?path=${encodeURIComponent(path)}`);
      if (!r.ok) return null;
      return await r.json();
    } catch { return null; }
  }, []);
  ...
  const writeFile = useCallback(async (path, content) => {
    ...
    try {
      const r = await fetch(`${api}/api/v1/runtime/file`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ path, content, message: "agent: live edit via chat" }),
      });
  ...
  const startAgent = useCallback(async (name) => {
    const api = getApiBase();
    try {
      const r = await fetch(`${api}/api/v1/agents/${name}/start`, { method: "POST" });
      if (r.ok) {
  ...
  const orchestrateAll = useCallback(async () => {
    ...
    setMessages((prev) => [...prev, {
      ...
      toolSteps: [{ label: "POST /runtime/agents/run-all", status: "running", timestamp: new Date().toISOString() }],
    }]);
    try {
      const r = await fetch(`${api}/api/v1/runtime/agents/run-all`, { method: "POST" });
      ...
      setMessages((prev) => [...prev, { role: "assistant", content: `❌ Orchestrate error: ${e.message}` }]);
    }
  }, []);
  ...
  const sendCommand = useCallback(async (text) => {
    ...
    if (msg === "status") {
      try {
        const [health, shadow] = await Promise.all([
          ...
          fetch(`${apiBase}/api/v1/runtime/shadow/status`).then((r) => r.json()).catch(() => null),
        ]);
        setMessages((prev) => [...prev, {
          role: "assistant",
          content: [
    ...
    if (editMatch) {
      ...
      if (file) {
        ...
        setMessages((prev) => [...prev, { role: "assistant", content: `❌ Cannot open editor — file not found: ${path}` }]);
      }
      setLoading(false);
      return;
    }
    ...
    if (msg.toLowerCase().includes("parallel") && msg.toLowerCase().includes("scrape")) {
      const cities = ["Miami FL", "Orlando FL", "Tampa FL", "Jacksonville FL"];
      const keyword = msg.toLowerCase().includes("epoxy") ? "epoxy contractors" : "flooring contractors";
      ...
      setMessages((prev) => [...prev, {
        ...
        toolSteps: cmds.map((c, i) => ({ label: `Worker ${i + 1}: ${c}`, status: "queued", timestamp: new Date().toISOString() })),
      }]);
    ...
    if (msg.toLowerCase().includes("vercel deploy") || msg.toLowerCase().includes("trigger deploy")) {
      try {
        ...
        setMessages((prev) => [...prev, { role: "assistant", content: `❌ Vercel error: ${e.message}` }]);
      }
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      setMessages((prev) => {
        const next = [...prev, {
          ...
          toolSteps: [{ label: `Dispatched to agent: ${data.agent}`, status: "running", timestamp: new Date().toISOString() }],
        }];
        return next;
      });
  ...
  }, [input, loading, pollTask, runParallel, orchestrateAll, readFile, messages.length]);
    ...
    <div style={S.container}>
      ...
      <div style={S.messages}>
        ...
        {loading && (
          <div style={{ ...S.messageRow, justifyContent: "flex-start" }}>
            ...
            <div style={{ ...S.bubble, background: "#111", border: "1px solid #222", borderRadius: "18px 18px 18px 4px" }}>
      ...
      {fileEditor && (
        <div style={S.fileEditorPanel}>
          <div style={S.fileEditorHeader}>
            <span style={{ color: "#FFD700", fontWeight: 700 }}>📝 Live Editor: {fileEditor.path}</span>
            <div style={{ display: "flex", gap: "0.5rem" }}>
              <button style={S.saveBtnGreen} onClick={saveFileEdit}>💾 Save & Deploy</button>
              <button style={S.cancelBtn} onClick={() => setFileEditor(null)}>✕ Close</button>
            </div>
          </div>
          <textarea
      ...
      <div style={S.workersBar}>
        <button style={S.capToggle} onClick={() => setShowWorkers((v) => !v)}>
          {showWorkers ? "▾" : "▸"} Workers ({workers.filter((w) => w.status === "running").length}/{workers.length})
        </button>
      ...
      {showAgents && agents.length > 0 && (
        <div style={S.agentsList}>
          {agents.map((a) => <AgentSidebarItem key={a.name} agent={a} onStart={startAgent} />)}
        </div>
      )}
          ...
          <>
            <div style={S.groupTabs}>
              {CAPABILITY_GROUPS.map((g, i) => (
                <button
                  key={i}
                  style={{ ...S.groupTab, ...(activeGroup === i ? S.groupTabActive : {}) }}
            ...
            <div style={S.cmdRow}>
              {CAPABILITY_GROUPS[activeGroup].cmds.map((cmd) => (
                <button key={cmd} style={S.chip} onClick={() => sendCommand(cmd)}>{cmd}</button>
              ))}
            </div>
          </>
        )}
      ...
      <div style={S.inputRow}>
        <textarea
          ...
          disabled={loading}
        />
        <button style={{ ...S.sendBtn, opacity: loading ? 0.5 : 1 }} onClick={() => sendCommand()} disabled={loading}>
          ➤
        </button>
      </div>
    </div>
...
const S = {
  container: {
    ...
    background: "#000", color: "#fff", fontFamily: "'Segoe UI', system-ui, sans-serif",
  },
  messages: {
  ...
  avatar: {
    ...
    border: "1px solid #FFD700", display: "flex", alignItems: "center",
  ...
  // Task panel
  taskPanel: {
    marginTop: "0.75rem", background: "#0d0d0d", border: "1px solid #222",
    borderRadius: "8px", padding: "0.75rem", fontSize: "0.8rem",
  },
  ...
  taskResult: {
    ...
    padding: "0.5rem", color: "#4ade80", fontSize: "0.75rem",
    maxHeight: "200px", overflowY: "auto", margin: 0, whiteSpace: "pre-wrap",
  },

  // Tool call steps
  toolStep: {
    marginTop: "0.3rem", background: "#0a0a0a", border: "1px solid #1a1a1a",
  ...
  toolStepDetail: {
    background: "#060606", color: "#4ade80", padding: "0.5rem 0.75rem",
    fontSize: "0.72rem", margin: 0, whiteSpace: "pre-wrap", borderTop: "1px solid #1a1a1a",
  },

  // File edit badge
  fileEditBadge: {
    marginTop: "0.5rem", background: "#0a0a0a", border: "1px solid #2a2a2a",
    borderRadius: "6px", padding: "0.4rem 0.75rem", fontSize: "0.78rem", color: "#aaa",
  },

  // Parallel workers
  workersBar: {
    display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.35rem 0.75rem",
    background: "#0a0a0a", borderTop: "1px solid #111", flexWrap: "wrap",
  },
  workersGrid: {
    display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "0.3rem",
    padding: "0.4rem 0.75rem", background: "#070707", borderTop: "1px solid #111",
  },
  workerSlot: {
    background: "#0d0d0d", border: "1px solid #333", borderRadius: "6px",
    padding: "0.4rem 0.6rem", minHeight: "60px",
  },
  stopBtn: {
    background: "transparent", border: "1px solid #f87171", color: "#f87171",
    borderRadius: "3px", width: "18px", height: "18px", fontSize: "0.6rem",
  ...
  agentsList: {
    ...
    padding: "0.3rem 0.75rem", display: "flex", flexDirection: "column", gap: "0.15rem",
    maxHeight: "180px", overflowY: "auto",
  },
  agentItem: {
    display: "flex", alignItems: "center", padding: "0.25rem 0.4rem",
    borderRadius: "4px", background: "#0d0d0d",
  },
  agentStartBtn: {
    background: "#1a1a1a", border: "1px solid #333", color: "#FFD700",
    borderRadius: "3px", width: "22px", height: "18px", fontSize: "0.65rem",
  ...
  shadowBadge: {
    ...
    borderBottom: "1px solid #1a1a1a", padding: "0.2rem 1rem",
    fontSize: "0.75rem", color: "#888",
  },

  // File live editor
  fileEditorPanel: {
    display: "flex", flexDirection: "column", background: "#070707",
    border: "1px solid #FFD700", borderRadius: "8px", margin: "0.5rem",
    maxHeight: "40vh", overflow: "hidden",
  },
  fileEditorHeader: {
    display: "flex", justifyContent: "space-between", alignItems: "center",
    padding: "0.5rem 0.75rem", background: "#0d0d0d", borderBottom: "1px solid #222",
    flexShrink: 0,
  },
  fileEditorTextarea: {
    flex: 1, background: "#080808", color: "#e2e8f0", border: "none", outline: "none",
    padding: "0.75rem", fontFamily: "'Fira Code', 'Cascadia Code', monospace",
    fontSize: "0.78rem", resize: "none", lineHeight: 1.5, overflowY: "auto",
  },
  saveBtnGreen: {
    background: "#4ade80", color: "#000", border: "none", borderRadius: "4px",
    padding: "0.3rem 0.75rem", cursor: "pointer", fontWeight: 700, fontSize: "0.8rem",
  },
  cancelBtn: {
    background: "#1a1a1a", color: "#aaa", border: "1px solid #333",
    borderRadius: "4px", padding: "0.3rem 0.6rem", cursor: "pointer", fontSize: "0.8rem",
  },

  // Cap bar
  capBar: {
    borderTop: "1px solid #111", padding: "0.5rem 1rem", background: "#0a0a0a",
    display: "flex", flexDirection: "column", gap: "0.5rem",
  },
  capToggle: {
  ...
  chip: {
    ...
    color: "#888", padding: "0.3rem 0.75rem", fontSize: "0.75rem",
  ...
  inputRow: {
    ...
    borderTop: "1px solid #1a1a1a", background: "#0a0a0a", alignItems: "flex-end",
  },
  input: {
    flex: 1, background: "#111", border: "1px solid #333", borderRadius: "12px",
    color: "#fff", padding: "0.75rem 1rem", fontSize: "0.9rem",
    fontFamily: "inherit", resize: "none", outline: "none", lineHeight: 1.5,
  },
  sendBtn: {
    background: "#FFD700", border: "none", borderRadius: "12px", color: "#000",
