...
function exportCSV(leads: Lead[]) {
  const headers = [
    "Company","City","State","Industry","Score","Tier","Phone","Email","Rating","Reviews",
  ];
  ...
  return (
    <div className="lead-card-mobile">
      ...
      <div className="flex items-center gap-3 mb-3 text-xs text-gray-500">
        ...
        {lead.rating != null && (
          <span className="flex-shrink-0">⭐ {lead.rating.toFixed(1)}{lead.reviews ? ` (${lead.reviews})` : ""}</span>
        )}
      </div>
    ...
    <div>
      ...
      <div className="glass-card rounded-t-2xl px-4 py-3 border-b border-yellow-400/10 flex items-center justify-between">
        <span className="text-sm text-gray-400">
          Showing{" "}
          <span className="text-white font-semibold">{displayLeads.length}</span>{" "}
          leads
        </span>
        <button
              ...
              {paginatedLeads.map((lead, idx) => (
                ...
                >
                  ...
                  <td className="px-5 py-3 text-xs text-gray-400 space-y-0.5">
                    {lead.phone && (
                      <div className="flex items-center gap-1">📞 {lead.phone}</div>
                    )}
                    {lead.email && (
                      <div className="flex items-center gap-1">✉️ {lead.email}</div>
                    )}
                  ...
                  <td className="px-5 py-3 whitespace-nowrap">
                    <div className="flex gap-1.5">
                      {lead.phone && (
                        <a href={`tel:${lead.phone}`} className="action-btn-gold">Call</a>
                      )}
                      {lead.email && (
                        <a
                      ...
                      <button className="action-btn-gold">Qualify</button>
                    </div>
                  </td>
                </tr>
              ))}
