...
const statusColor = (s: string) => {
  if (s === "success") return "text-green-400 bg-green-400/10 border border-green-400/30";
  if (s === "failure") return "text-red-400 bg-red-400/10 border border-red-400/30";
  ...
  return (
    <div className="space-y-6 fade-in">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-white">GitHub Actions Workflows</h2>
        <a
      ...
      <div className="grid grid-cols-1 gap-4">
        {WORKFLOWS.map((wf) => (
          <div
            ...
            onClick={() => { setSelectedWorkflow(wf); setShowYaml(false); }}
          >
            <div className="flex items-start justify-between mb-3">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <h3 className="text-sm font-semibold text-white">{wf.name}</h3>
                  <span className={`text-xs px-2 py-0.5 rounded-full ${statusColor(wf.lastStatus)}`}>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3 text-xs text-gray-500">
                ...
                <span className="font-mono text-[10px] text-gray-600">{wf.file}</span>
              </div>
              <div className="flex gap-2">
                <button
                  onClick={(e) => { e.stopPropagation(); setSelectedWorkflow(wf); setShowYaml(true); }}
          ...
          <div className="bg-[#0a0a0a] border border-[#2a2a2a] rounded-xl w-full max-w-2xl max-h-[80vh] flex flex-col">
            <div className="flex items-center justify-between px-5 py-4 border-b border-[#2a2a2a]">
              <div>
                ...
                <span className="text-xs text-gray-500 font-mono">{selectedWorkflow.file}</span>
              </div>
              <button
