    ...
    <div style={S.page}>
      <div style={S.header}>
        ...
        <div style={S.headerLinks}>
          ...
          <Link href="/workspace" style={S.navLink}>Workspace</Link>
          ...
          <>
            ...
            <div style={S.section}>
              ...
              <div style={S.statGrid}>
                <StatCard label="Status" value={health?.status || "—"} color={health?.status === "healthy" ? "#4ade80" : "#f87171"} />
                <StatCard label="Service" value={health?.service || "—"} color="#7dd3fc" />
                <StatCard label="Version" value={health?.version || "—"} color="#c4b5fd" />
                <StatCard label="Workers" value={metrics?.workers?.alive ?? "—"} color="#fbbf24" />
              </div>
            </div>
            ...
            <div style={S.section}>
              ...
              <div style={S.statGrid}>
                <StatCard label="Total Tasks" value={tasks?.total ?? 0} color="#FFD700" />
            ...
            {metrics?.queue && (
              <div style={S.section}>
                ...
                <div style={S.statGrid}>
                  <StatCard label="Queue Total" value={metrics.queue.total ?? 0} color="#a78bfa" />
                  {Object.entries(metrics.queue.queues || {}).map(([q, cnt]) => (
                    <StatCard key={q} label={`Queue: ${q}`} value={cnt} color="#fb923c" />
                  ))}
                </div>
              </div>
            )}
              ...
              <div style={S.section}>
                ...
                <div style={S.tableWrapper}>
                  <table style={S.table}>
                    <thead>
                      <tr>
                        {["Task ID", "Agent", "Type", "Status", "Created"].map((h) => (
                          <th key={h} style={S.th}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {taskList.slice(-10).reverse().map((t) => (
                        <tr key={t.task_id} style={S.tr}>
                          <td style={S.td}><code>{t.task_id?.slice(0, 8)}</code></td>
                          <td style={S.td}>{t.agent || "—"}</td>
                          <td style={S.td}>{t.command_type || "—"}</td>
                          <td style={{ ...S.td, color: t.status === "completed" ? "#4ade80" : t.status === "failed" ? "#f87171" : "#fbbf24" }}>
                            {t.status}
                          </td>
                          <td style={S.td}>{t.created_at ? new Date(t.created_at).toLocaleTimeString() : "—"}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
...
const S = {
  ...
  card: { background: "#0a0a0a", border: "1px solid #1a1a1a", borderRadius: "8px", padding: "1.25rem", textAlign: "center" },
  cardValue: { fontSize: "2rem", fontWeight: 800 },
  cardLabel: { color: "#555", fontSize: "0.8rem", marginTop: "0.25rem", textTransform: "uppercase", letterSpacing: "0.05em" },
  tableWrapper: { overflowX: "auto" },
  table: { width: "100%", borderCollapse: "collapse", fontSize: "0.875rem" },
  th: { background: "#0a0a0a", color: "#666", padding: "0.6rem 1rem", textAlign: "left", borderBottom: "1px solid #1a1a1a", fontSize: "0.75rem", textTransform: "uppercase" },
