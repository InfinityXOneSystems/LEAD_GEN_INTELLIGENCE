...
export default function LeadsPage() {
  ...
  const exportCsv = () => {
    const cols = ["company_name", "phone", "website", "email", "city", "state", "lead_score", "rating"];
    ...
    <div style={styles.page}>
      ...
      <div style={styles.content}>
        <div style={styles.toolbar}>
          <h1 style={styles.title}>📋 Leads <span style={{ color: "#888", fontSize: "1rem" }}>({total})</span></h1>
        ...
        {loading ? (
          <div style={{ color: "#FFD700", padding: "2rem", textAlign: "center" }}>Loading…</div>
        ) : (
          <div style={{ overflowX: "auto" }}>
            <table style={styles.table}>
              <thead>
                <tr>
                  {["Company", "City", "State", "Score", "Phone", "Website", "Rating"].map((h) => (
              ...
              <tbody>
                {filtered.map((l, i) => (
                  <tr key={i} style={{ borderBottom: "1px solid #0f0f0f" }}>
                    ...
                    <td style={{ ...styles.td, color: "#aaa" }}>{l.city || "—"}</td>
                    <td style={{ ...styles.td, color: "#aaa" }}>{l.state || "—"}</td>
                    <td style={{ ...styles.td, color: "#FFD700", fontWeight: 600 }}>{l.lead_score ?? l.score ?? 0}</td>
                    <td style={{ ...styles.td, color: "#4CAF50" }}>{l.phone || "—"}</td>
                    <td style={styles.td}>
                      {l.website ? (
                        <a href={l.website} target="_blank" rel="noopener noreferrer" style={{ color: "#2196F3" }}>🔗 site</a>
                      ) : "—"}
                    </td>
                    <td style={{ ...styles.td, color: "#aaa" }}>{l.rating ? `⭐ ${l.rating}` : "—"}</td>
                  </tr>
                ))}
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={7} style={{ ...styles.td, textAlign: "center", color: "#555", padding: "2rem" }}>
                      No leads found. Try scraping some leads via the{" "}
                      <a href="/chat" style={{ color: "#FFD700" }}>Chat interface</a>.
                    </td>
                  </tr>
                )}
              </tbody>
        ...
        <div style={styles.pagination}>
          ...
          <span style={{ color: "#888", fontSize: "0.875rem" }}>Page {page + 1}</span>
          <button
            style={{ ...styles.pageBtn, opacity: leads.length < PAGE_SIZE ? 0.3 : 1 }}
...
const styles = {
  ...
  filterInput: {
    ...
    color: "#fff", padding: "0.4rem 0.75rem", fontSize: "0.875rem",
    fontFamily: "inherit", outline: "none", width: "150px",
  },
  exportBtn: {
    background: "transparent", border: "1px solid #FFD700", borderRadius: "6px",
    color: "#FFD700", padding: "0.4rem 0.75rem", fontSize: "0.875rem",
    cursor: "pointer", fontFamily: "inherit",
  },
  table: { width: "100%", borderCollapse: "collapse", fontSize: "0.85rem" },
  th: { padding: "0.6rem 0.5rem", textAlign: "left", color: "#FFD700", borderBottom: "1px solid #222", whiteSpace: "nowrap" },
  td: { padding: "0.45rem 0.5rem", verticalAlign: "middle" },
  pagination: { display: "flex", alignItems: "center", gap: "1rem", padding: "1rem 0", justifyContent: "center" },
  pageBtn: { background: "transparent", border: "1px solid #333", borderRadius: "6px", color: "#fff", padding: "0.4rem 0.75rem", cursor: "pointer", fontFamily: "inherit", fontSize: "0.875rem" },
};
