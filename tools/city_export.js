...
const leadsFile       = path.resolve(__dirname, '..', 'data', 'leads', 'scored_leads.json');
const fallbackFile    = path.resolve(__dirname, '..', 'data', 'leads', 'leads.json');
...
const CSV_HEADERS = ['company', 'phone', 'email', 'website', 'address', 'city', 'state', 'rating', 'reviews', 'lead_score', 'tier'];
...
for (const { city, state } of TARGET_CITIES) {
  ...
  const filtered = leads.filter(l => {
    ...
    return lCity.toLowerCase() === city.toLowerCase() && lState === state.toUpperCase();
  });
  ...
  const csvContent = [CSV_HEADERS.join(','), ...filtered.map(buildCsvRow)].join('\n') + '\n';
    ...
    filtered.length > 0
      ? `## Top Leads\n\n| Company | Phone | Website | Score |\n|---------|-------|---------|-------|\n` +
        filtered
          ...
          .slice(0, 20)
          .map(l => `| ${escapeCsvField(l.company)} | ${l.phone || ''} | ${l.website || ''} | ${l.lead_score || 0} |`)
          .join('\n')
  ...
  console.log(`[city_export] ${city}, ${state}: ${filtered.length} leads → ${csvPath}`);
}
