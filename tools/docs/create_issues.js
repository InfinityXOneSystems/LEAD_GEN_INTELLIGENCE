...
const GITHUB_REPOSITORY = process.env.GITHUB_REPOSITORY || 'InfinityXOneSystems/LEAD_GEN_INTELLIGENCE';
...
async function ghRequest(method, endpoint, body) {
  if (!GITHUB_TOKEN) {
    console.warn('[create_issues] GITHUB_TOKEN not set — skipping GitHub API calls');
    return null;
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error(`[create_issues] GitHub API error ${res.status}: ${text.slice(0, 200)}`);
      return null;
    }
...
async function ensureLabel() {
  ...
  const existing = await ghRequest('GET', `/repos/${OWNER}/${REPO}/labels/${encodeURIComponent(LABEL_NAME)}`);
...
async function searchExistingIssues(fingerprint) {
  ...
  const result = await ghRequest('GET', `/search/issues?q=${encodeURIComponent(query)}&per_page=5`);
...
async function main() {
  ...
  try {
    ...
    console.error('[create_issues] Failed to parse self_review.json:', e.message);
    return;
  }

  const recommendations = (data.recommendations || []).filter(
    r => CREATE_FOR_SEVERITIES.has(r.severity)
  );

  if (!recommendations.length) {
    console.log('[create_issues] No HIGH/MEDIUM recommendations to create issues for.');
    return;
  }

  if (!GITHUB_TOKEN) {
    console.log('[create_issues] No GITHUB_TOKEN — would create issues for:');
    recommendations.forEach(r => console.log(`  - [${r.severity}] ${r.title}`));
    return;
  }
  ...
  for (const rec of recommendations) {
    ...
    if (exists) {
      console.log(`[create_issues] Skipping (duplicate): ${rec.title} [${rec.fingerprint}]`);
      skipped++;
      continue;
    }
  ...
  console.log(`[create_issues] ✅ Done. Created: ${created}, Skipped (duplicates): ${skipped}`);
}
