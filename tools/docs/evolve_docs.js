...
const EXCLUDE_DIRS = new Set([
  '.git', 'node_modules', '.next', 'out', '__pycache__',
  '.turbo', '.vercel', 'dist', 'build', 'coverage'
]);

// File extensions to include in TODO scan
const TODO_EXTENSIONS = new Set([
  '.js', '.ts', '.tsx', '.jsx', '.py', '.sh', '.yml', '.yaml', '.json', '.sql'
]);
...
function safeExec(cmd, cwd) {
  try {
    return execSync(cmd, { cwd: cwd || ROOT, encoding: 'utf8', timeout: 15000 }).trim();
  } catch {
    return null;
  }
}
...
function getKeyEntrypoints() {
  const entrypoints = [
    { path: 'agents/orchestrator/orchestrator.js', desc: 'Master pipeline controller' },
    { path: 'agents/scoring/scoring_pipeline.js', desc: 'Lead scoring pipeline' },
    { path: 'agents/scoring/lead_scoring.js', desc: 'Scoring engine (0–100)' },
    { path: 'agents/gpt_actions/server.js', desc: 'GPT Actions REST API server' },
    ...
    { path: 'tools/docs/evolve_docs.js', desc: 'Living docs generator (this file)' },
...
function scanTodos() {
  ...
  function scanDir(dir) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        scanDir(fullPath);
      } else if (TODO_EXTENSIONS.has(path.extname(entry.name)) && !TODO_SKIP_PATHS.has(fullPath)) {
        try {
            ...
            if (match) {
              ...
              if (text) {
                todos.push({
                  ...
                  line: i + 1,
                  text,
                  priority: text.toLowerCase().includes('critical') || text.toLowerCase().includes('urgent') ? 'HIGH' :
...
function detectLabel(text, filename) {
  ...
  if (t.includes('security') || t.includes('auth') || t.includes('secret')) return 'security';
  ...
  if (t.includes('performance') || t.includes('slow') || t.includes('optimize')) return 'performance';
...
function getSystemStatus() {
  ...
  // Scored leads
  const scoredLeads = readJsonSafe(path.join(ROOT, 'data', 'leads', 'scored_leads.json'));
  const scoringReport = readJsonSafe(path.join(ROOT, 'data', 'leads', 'scoring_report.json'));

  if (scoredLeads) {
    status.leadsCount = Array.isArray(scoredLeads) ? scoredLeads.length : 0;
    const stat = fileStat(path.join(ROOT, 'data', 'leads', 'scored_leads.json'));
  ...
  // Outreach log
  const outreachLog = readJsonSafe(path.join(ROOT, 'data', 'outreach', 'outreach_log.json'));
  ...
  // Scraper progress
  const scraperProgress = readJsonSafe(path.join(ROOT, 'data', 'scraper_progress.json'));
  status.scraperProgress = scraperProgress || 'No progress file found';

  // Git info
  status.lastCommit = safeExec('git log -1 --format="%H %s" 2>/dev/null') || 'unknown';
  status.branch = safeExec('git rev-parse --abbrev-ref HEAD 2>/dev/null') || 'unknown';
  status.commitCount = safeExec('git rev-list --count HEAD 2>/dev/null') || 'unknown';
...
function runSelfReview(todos, status) {
  ...
  if (status.leadsCount === 0) {
    recommendations.push({
      ...
      action: 'Run the lead scraper pipeline: trigger `.github/workflows/lead_scraper.yml` or run `npm run score` locally.',
  ...
  if (highPriorityTodos.length > 0) {
    recommendations.push({
      ...
      action: `Review and resolve: ${highPriorityTodos.slice(0, 3).map(t => t.file + ':' + t.line).join(', ')}`,
    ...
    try {
      ...
      if (content.includes('TODO') && content.includes('nodemailer')) {
        recommendations.push({
          ...
          action: 'Configure SMTP credentials and complete nodemailer setup in outreach/outreach_engine.js.',
  ...
  if (fs.existsSync(testsDir)) {
    try {
      testFileCount = fs.readdirSync(testsDir).filter(f => f.endsWith('.test.js')).length;
    } catch { /* skip */ }
  }
  if (testFileCount < 3) {
    recommendations.push({
      ...
      action: 'Add test files for outreach_engine.js, google_maps_scraper.js, and enrichment agents.',
    ...
    try {
      ...
      if (content.length < 500) {
        recommendations.push({
          ...
          description: 'README.md contains less than 500 characters and does not describe the platform.',
          action: 'Expand README.md with quickstart, architecture overview, and links to docs/.',
  ...
  if (!fs.existsSync(envExample)) {
    recommendations.push({
      ...
      description: '.env.example was not found. Contributors cannot onboard without knowing required environment variables.',
      action: 'Create .env.example with all required environment variable names (no values).',
...
function generateChangelogUpdate() {
  ...
  const recentCommits = safeExec('git log --oneline -20 --no-merges 2>/dev/null');
...
function main() {
  ...
  console.log(`[evolve_docs] Generated ${recommendations.length} recommendations`);
  ...
  writeDoc(path.join(DOCS, 'SELF_REVIEW.md'), generateSelfReviewMd(recommendations, todos, status));
  writeDoc(path.join(DOCS, 'self_review.json'), generateSelfReviewJson(recommendations));
  ...
  if (recentCommits && fs.existsSync(changelogPath)) {
    ...
    if (!existing.includes('## [Unreleased — Recent Commits]')) {
      ...
      const updated = existing.replace('## [Unreleased]', `## [Unreleased]${section}`);
      if (updated !== existing) {
        writeDoc(changelogPath, updated);
      } else {
        console.log('[evolve_docs] CHANGELOG.md unchanged (section already present)');
      }
    } else {
      console.log('[evolve_docs] CHANGELOG.md already has recent commits section');
    }
  }
