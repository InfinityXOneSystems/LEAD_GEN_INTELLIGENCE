...
app.get('/api/leads', (req, res) => {
  try {
    ...
    if (city) result = result.filter((l) => l.city && l.city.toLowerCase().includes(city.toLowerCase()));
    if (state) result = result.filter((l) => l.state && l.state.toLowerCase() === state.toLowerCase());
    if (minScore) result = result.filter((l) => (l.score || 0) >= Number(minScore));
...
app.get('/api/leads/:id', (req, res) => {
  try {
    ...
    const lead = list.find((l) => String(l.id) === req.params.id || l.place_id === req.params.id);
...
app.get('/api/pipeline/status', (req, res) => {
  try {
    ...
    return ok(res, { status: healthy ? 'healthy' : 'degraded', checks, lastRun });
  } catch (err) {
    return fail(res, err.message);
  }
});
...
app.get('/api/scoring/report', (req, res) => {
  try {
    ...
    if (!fs.existsSync(reportFile)) return fail(res, 'Scoring report not found', 404);
...
app.post('/api/outreach/send', async (req, res) => {
  ...
  try {
    const leads = loadLeads();
    const list = Array.isArray(leads) ? leads : [];
    const lead = list.find((l) => String(l.id) === String(leadId) || l.place_id === String(leadId));
    if (!lead) return fail(res, 'Lead not found', 404);
    ...
    if (fs.existsSync(logFile)) {
      try { queue = readJson(logFile); } catch (_) {}
    }
    queue.push(entry);
...
app.get('/api/heatmap', (req, res) => {
  try {
    ...
    list.forEach((l) => {
      ...
      if (!cityMap[key]) cityMap[key] = { city: l.city, state: l.state, count: 0, totalScore: 0, leads: [] };
      cityMap[key].count += 1;
      cityMap[key].totalScore += l.score || 0;
      if (cityMap[key].leads.length < 5) cityMap[key].leads.push({ id: l.id || l.place_id, name: l.name || l.company_name });
    });

    const heatmap = Object.values(cityMap).map((c) => ({
      ...c,
      avgScore: c.count ? Math.round(c.totalScore / c.count) : 0,
    })).sort((a, b) => b.count - a.count);

    return ok(res, { heatmap, totalCities: heatmap.length, totalLeads: list.length });
  } catch (err) {
    return fail(res, err.message);
  }
});
