...
function validateLeadSchema(lead) {
  const errors = [];
  if (!lead || typeof lead !== 'object') { errors.push('Lead must be an object'); return errors; }

  const name = lead.name || lead.company_name || lead.company;
  if (!name || typeof name !== 'string' || name.trim() === '') errors.push('name/company_name/company is required');
  ...
  if (lead.website !== undefined && lead.website !== null) {
    if (typeof lead.website !== 'string') errors.push('website must be a string');
  }
...
describe('Leads Data Integration', () => {
  test('leads/leads.json or leads/scored_leads.json exists', () => {
    ...
    assert.ok(eitherExists, 'At least one leads file (leads.json or scored_leads.json) must exist');
  });
  ...
  test('leads file contains a valid JSON array', () => {
    ...
    const file = fileExists(scoredFile) ? scoredFile : (fileExists(rawFile) ? rawFile : null);
...
describe('Lead Schema Validation', () => {
  ...
  test('validateLeadSchema flags missing name', () => {
    ...
    assert.ok(errors.some((e) => e.includes('name')), 'Error should mention name field');
  });

  test('validateLeadSchema flags invalid score type', () => {
    const lead = { name: 'Test Co', score: '45' };
    const errors = validateLeadSchema(lead);
    assert.ok(errors.some((e) => e.includes('score')), 'Error should mention score field');
  });
...
describe('Scoring Pipeline Output', () => {
  ...
  test('scored leads score range is 0–100', () => {
    ...
    assert.equal(outOfRange.length, 0, `${outOfRange.length} leads have scores outside 0–100`);
  });
});
...
describe('API Gateway', () => {
  ...
  test('API gateway /api/stats returns valid JSON envelope', async () => {
    if (!(await gatewayAvailable())) {
      console.log('  ⚠ Skipping: API gateway not running on port ' + GATEWAY_PORT);
      return;
    }
  ...
  test('API gateway /api/leads returns array of leads', async () => {
    ...
    assert.ok(typeof body.data.total === 'number', 'data.total must be a number');
  });
  ...
  test('API gateway /api/leads/:id 404 for unknown id', async () => {
    ...
    const { statusCode, body } = await httpGet(`${BASE_URL}/api/leads/nonexistent-id-xyz`);
