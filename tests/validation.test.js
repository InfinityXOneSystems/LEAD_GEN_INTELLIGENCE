...
test('validateLead - score calculation partial fields', () => {
  ...
  assert.ok(result.score > 0, 'score should be positive when fields are present');
  assert.ok(result.score < 100, 'score should be less than 100 when fields are missing');
});
...
test('dedupe - no duplicates returns all as unique', () => {
  const leads = [
    { company: 'Alpha', city: 'Columbus', phone: '6145550001', email: 'a@a.com' },
    { company: 'Beta',  city: 'Cleveland', phone: '6145550002', email: 'b@b.com' },
  ];
