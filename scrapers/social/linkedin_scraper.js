...
async function scrapeLinkedInCompany(companySlug) {
  ...
  try {
    ...
    const tagline = await page.$eval('.org-top-card-summary__tagline', el => el.textContent.trim()).catch(() => '');
    const website = await page.$eval('[data-field="website"] a', el => el.href).catch(() => '');
    const industry = await page.$eval('.org-top-card-summary-info-list__info-item', el => el.textContent.trim()).catch(() => '');
    const size     = await page.$$eval('.org-about-company-module__company-size-definition-text span',
      els => els.map(e => e.textContent.trim()).join(' ')).catch(() => '');
    const description = await page.$eval('.org-about-us-organization-description__text', el => el.textContent.trim()).catch(() => '');
...
if (require.main === module) {
  ...
  if (!slugs.length) { console.error('Usage: node linkedin_scraper.js <slug> [slug2 ...]'); process.exit(1); }
  scrapeLinkedInBatch(slugs).then(r => console.log(JSON.stringify(r, null, 2)));
}
