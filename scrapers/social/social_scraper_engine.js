...
function mergeSocialData(existingLeads, socialResults) {
  const byName = new Map(existingLeads.map(l => [l.company?.toLowerCase().trim(), l]));
  ...
  for (const social of socialResults) {
    ...
    if (lead) {
      ...
      if (!lead.linkedinUrl  && social.source === 'linkedin')  lead.linkedinUrl  = social.profileUrl;
      if (!lead.twitterUrl   && social.source === 'twitter')   lead.twitterUrl   = social.profileUrl;
      if (!lead.facebookUrl  && social.source === 'facebook')  lead.facebookUrl  = social.profileUrl;
      if (!lead.instagramUrl && social.source === 'instagram') lead.instagramUrl = social.profileUrl;
...
async function run(targets = {}) {
  ...
  const jobs = [];
  if (targets.linkedin?.length)  jobs.push(scrapeLinkedInBatch(targets.linkedin, concurrency).then(r => allResults.push(...r)));
  if (targets.twitter?.length)   jobs.push(scrapeTwitterBatch(targets.twitter, concurrency).then(r => allResults.push(...r)));
  if (targets.facebook?.length)  jobs.push(scrapeFacebookBatch(targets.facebook, concurrency).then(r => allResults.push(...r)));
  if (targets.instagram?.length) jobs.push(scrapeInstagramBatch(targets.instagram, concurrency).then(r => allResults.push(...r)));
  ...
  console.log(`[SocialEngine] Saved ${enriched.length} leads (${allResults.length} social records)`);
...
if (require.main === module) {
  ...
  if (!targets.linkedin.length && !targets.twitter.length && !targets.facebook.length && !targets.instagram.length) {
    console.log('[SocialEngine] No targets specified.  Set SOCIAL_LINKEDIN, SOCIAL_TWITTER, SOCIAL_FACEBOOK, or SOCIAL_INSTAGRAM env vars.');
    console.log('Example: SOCIAL_LINKEDIN=acme-flooring,bob-tile node scrapers/social/social_scraper_engine.js');
    process.exit(0);
  }
