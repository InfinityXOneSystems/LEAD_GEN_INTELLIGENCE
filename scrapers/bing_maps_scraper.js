...
async function scrapeBingMaps(keyword, city, state) {
  ...
  try {
    ...
    try {
      ...
      console.warn(`[bing_maps] No results list found for: ${keyword} in ${city}, ${state}`);
      return leads;
    }
    ...
    const rawLeads = await page.evaluate(
      ({ max, kw }) => {
        const cards = Array.from(
          document.querySelectorAll('.entity-card, .listing-card, [class*="entityCard"]')
        ).slice(0, max);

        return cards.map((card) => {
          const company =
            card.querySelector('a[class*="title"], .entityCard-title, h2')?.textContent?.trim() || '';
          const phone =
            card.querySelector('[class*="phone"], [data-cid*="phone"]')?.textContent?.trim() || '';
          const website = card.querySelector('a[class*="website"], a[href*="http"]')?.href || '';
          const address = card.querySelector('[class*="address"], .address')?.textContent?.trim() || '';
          const ratingText = card.querySelector('[class*="rating"]')?.textContent?.trim() || '';
          const ratingMatch = ratingText.match(/([\d.]+)/);
          const rating = ratingMatch ? parseFloat(ratingMatch[1]) : 0;
          const reviewsText =
            card.querySelector('[class*="review"]')?.textContent?.replace(/,/g, '') || '';
          const reviewsMatch = reviewsText.match(/(\d+)/);
          const reviews = reviewsMatch ? parseInt(reviewsMatch[1], 10) : 0;
          const category =
            card.querySelector('[class*="category"]')?.textContent?.trim() || '';

          return { company, phone, website, address, rating, reviews, category, keyword: kw };
        });
      },
    ...
    console.error(`[bing_maps] Fatal error for "${keyword}" in ${city}, ${state}: ${err.message}`);
  } finally {
    await browser.close();
  }

  console.log(`[bing_maps] Collected ${leads.length} leads for "${keyword}" in ${city}, ${state}`);
  return leads;
}
